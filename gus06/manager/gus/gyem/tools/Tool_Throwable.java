...
public class Tool_Throwable {
	...
	public static String location(Throwable e)
	{
		...
		if(ste_.length==0) return null;
		int n=0;
		StackTraceElement ste = ste_[0];
		while(ste.getFileName()==null && ste_.length>n+1) ste = e.getStackTrace()[++n];
		return location(ste);
	}
	...
	public static StackTraceElement firstEntitySTE(Throwable e)
	{
		...
		if(ste[i].getClassName().startsWith("gus06.entity.")) return ste[i];
		return null;
	}
