package gus06.entity.gus.sys.webserver1.web3.processor2.prepare.session;
...
public class EntityImpl implements Entity, T {
	...
	{
		accessBuilder = Outside.service(this,"gus.sys.webserver1.web3.processor2.prepare.session.access");
	...
	{
		Date date = new Date();
		long expireTime = EXPIRETIME; // possibilité de paramétrer à partir d'une propriété ?
		Map session = new HashMap();
		
		session.put(KEY_DATESTART,date);
		session.put(KEY_DATELAST,date);
		session.put(KEY_EXPIRETIME,new Long(expireTime));
