...
public class Inno_ModelPPM
{
	...
	private Inno_SEE2Context[][] SEE2Cont = new Inno_SEE2Context[25][16];

	private Inno_SEE2Context dummySEE2Cont;

	private Inno_PPMContext minContext, medContext, maxContext;

	private Inno_State foundState; // found next state transition
	...
	private Inno_RangeCoder coder = new Inno_RangeCoder();

	private Inno_SubAllocator subAlloc = new Inno_SubAllocator();
    ...
    private final Inno_StateRef tempStateRef1 = new Inno_StateRef();
    private final Inno_StateRef tempStateRef2 = new Inno_StateRef();
    private final Inno_PPMContext tempPPMContext1 = new Inno_PPMContext(null);
    private final Inno_PPMContext tempPPMContext2 = new Inno_PPMContext(null);
    private final Inno_PPMContext tempPPMContext3 = new Inno_PPMContext(null);
    private final Inno_PPMContext tempPPMContext4 = new Inno_PPMContext(null);
    private final int[] ps = new int[MAX_O];

	public Inno_ModelPPM()
	{
		minContext = null;
		maxContext = null;
		medContext = null;
	...
	public Inno_SubAllocator getSubAlloc()
	{
		return subAlloc;
	}
	...
	private void restartModelRare()
	{
		...
		Inno_State state = new Inno_State(subAlloc.getHeap());
		addr = minContext.getFreqData().getStats();
        runLength = initRL;
        prevSuccess = 0;
		for (int i = 0; i < 256; i++) {
			state.setAddress(addr + i * Inno_State.size);
			state.setSymbol(i);
			state.setFreq(1);
	...
	public boolean decodeInit(Inno_Unpack unpackRead, int escChar/* ref */)
			throws IOException, Inno_RarException
	{
		...
		if (reset) {
			...
			minContext = new Inno_PPMContext(getHeap());
			medContext = new Inno_PPMContext(getHeap());
			maxContext = new Inno_PPMContext(getHeap());
			foundState = new Inno_State(getHeap());
			dummySEE2Cont = new Inno_SEE2Context();
			for (int i = 0; i < 25; i++) {
				for (int j = 0; j < 16; j++) {
					SEE2Cont[i][j] = new Inno_SEE2Context();
				}
			}
	...
	public int decodeChar() throws IOException, Inno_RarException
	{
        // Debug
        //subAlloc.dumpHeap();
	...
	public Inno_SEE2Context[][] getSEE2Cont()
	{
		return SEE2Cont;
	}

	public Inno_SEE2Context getDummySEE2Cont()
	{
	...
	public Inno_RangeCoder getCoder()
	{
		return coder;
	}
	...
	public Inno_State getFoundState()
	{
		return foundState;
	}
	...
	private int /* ppmcontext ptr */createSuccessors(boolean Skip,
            Inno_State p1 /* state ptr */) {
		//State upState = tempState1.init(null);
		Inno_StateRef upState = tempStateRef2;
		Inno_State tempState = tempState1.init(getHeap());

		// PPM_CONTEXT* pc=MinContext, * UpBranch=FoundState->Successor;
		Inno_PPMContext pc = tempPPMContext1.init(getHeap());
		pc.setAddress(minContext.getAddress());
		Inno_PPMContext upBranch = tempPPMContext2.init(getHeap());
		upBranch.setAddress(foundState.getSuccessor());

		// STATE * p, * ps[MAX_O], ** pps=ps;
		Inno_State p = tempState2.init(getHeap());
		int pps = 0;
	...
	private void updateModel()
	{
		...
		Inno_StateRef fs = tempStateRef1;
		fs.setValues(foundState);
		Inno_State p = tempState3.init(getHeap());
		Inno_State tempState = tempState4.init(getHeap());

		Inno_PPMContext pc = tempPPMContext3.init(getHeap());
		Inno_PPMContext successor = tempPPMContext4.init(getHeap());
		...
		if (fs.getFreq() < MAX_FREQ / 4 && pc.getAddress() != 0) {
			if (pc.getNumStats() != 1) {
				...
				if (p.getSymbol() != fs.getSymbol()) {
					...
					tempState.setAddress(p.getAddress() - Inno_State.size);
					if (p.getFreq() >= tempState.getFreq()) {
						Inno_State.ppmdSwap(p, tempState);
						p.decAddress();
					}
				}
...
//								.getSummFreq() <= 8 * ns1) ? 1 : 0));
			...
			p.setAddress(pc.getFreqData().getStats() + ns1*Inno_State.size);
