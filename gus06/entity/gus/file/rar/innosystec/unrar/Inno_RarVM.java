...
public class Inno_RarVM extends Inno_BitInput {
	...
	private boolean isVMMem(byte[] mem) {return this.mem == mem;}

	private int getValue(boolean byteMode, byte[] mem, int offset)
	{
		if(byteMode)
		{
			if(isVMMem(mem)) return (mem[offset]);
			return (mem[offset] & 0xff);
		}
		else
		{
			if (isVMMem(mem)) return Inno_Raw.readIntLittleEndian(mem, offset);
			return Inno_Raw.readIntBigEndian(mem, offset);
		}
	}
	...
	private void setValue(boolean byteMode, byte[] mem, int offset, int value)
	{
		...
		else
		{
			if(isVMMem(mem))
			{
				Inno_Raw.writeIntLittleEndian(mem, offset, value);
...
//				mem[offset + 3] = (byte) (value >>> 24);
			...
			{
				Inno_Raw.writeIntBigEndian(mem, offset, value);
...
//		mem[offset + 3] = (byte) ((value >>> 24)&0xff);
	...
	private int getOperand(Inno_VMPreparedOperand cmdOp)
	{
		int ret = 0;
		if (cmdOp.getType() == Inno_VMOpType.VM_OPREGMEM)
		{
			int pos = (cmdOp.getOffset() + cmdOp.getBase()) & VM_MEMMASK;
			ret = Inno_Raw.readIntLittleEndian(mem, pos);
		}
		else
		{
	...
	public void execute(Inno_VMPreparedProgram prg)
	{
		...
		List<Inno_VMPreparedCommand> preparedCode = prg.getAltCmd().size() != 0 ? prg.getAltCmd(): prg.getCmd();

		if (!ExecuteCode(preparedCode, prg.getCmdCount()))
		preparedCode.get(0).setOpCode(Inno_VMCommands.VM_RET);
	...
	private boolean ExecuteCode(List<Inno_VMPreparedCommand> preparedCode, int cmdCount)
	{
		maxOpCount = 25000000;
		...
		while(true)
		{
			Inno_VMPreparedCommand cmd = preparedCode.get(IP);
			...
			case VM_CMP: {
				...
				if(result == 0) flags = Inno_VMFlags.VM_FZ.getFlag();
				else flags = (result > value1) ? 1 : 0 | (result & Inno_VMFlags.VM_FS.getFlag());
			...
			case VM_CMPB: {
				...
				if (result == 0) {
					flags = Inno_VMFlags.VM_FZ.getFlag();
				} else {
					flags = (result > value1) ? 1 : 0 | (result & Inno_VMFlags.VM_FS
							.getFlag());
				}
			}
				break;
			...
			case VM_ADD: {
				...
				if (cmd.isByteMode()) {
					...
					flags = (result < value1) ? 1
							: 0 | (result == 0 ? Inno_VMFlags.VM_FZ.getFlag()
									: ((result & 0x80) != 0) ? Inno_VMFlags.VM_FS
											.getFlag() : 0);
					...
					flags = (result < value1) ? 1
							: 0 | (result == 0 ? Inno_VMFlags.VM_FZ.getFlag()
									: (result & Inno_VMFlags.VM_FS.getFlag()));
			...
			case VM_SUB: {
				...
				flags = (result == 0) ? Inno_VMFlags.VM_FZ.getFlag()
						: (result > value1) ? 1 : 0 | (result & Inno_VMFlags.VM_FS
								.getFlag());
			...
			case VM_JZ:
				if ((flags & Inno_VMFlags.VM_FZ.getFlag()) != 0) {
			...
			case VM_INC: {
				...
				setValue(cmd.isByteMode(), mem, op1, result);
				flags = result == 0 ? Inno_VMFlags.VM_FZ.getFlag() : result
						& Inno_VMFlags.VM_FS.getFlag();
			}
				break;
			...
			case VM_TEST: {
				...
				break;
			case VM_JS:
				if ((flags & Inno_VMFlags.VM_FS.getFlag()) != 0) {
					setIP(getValue(false, mem, op1));
					continue;
				}
				break;
			case VM_JNS:
				...
				break;
			case VM_JB:
				if ((flags & Inno_VMFlags.VM_FC.getFlag()) != 0) {
					setIP(getValue(false, mem, op1));
					continue;
				}
				break;
			case VM_JBE:
				if ((flags & (Inno_VMFlags.VM_FC.getFlag() | Inno_VMFlags.VM_FZ.getFlag())) != 0) {
					setIP(getValue(false, mem, op1));
					continue;
				}
				break;
			...
			case VM_SHL: {
				...
				flags = (result == 0 ? Inno_VMFlags.VM_FZ.getFlag()
						: (result & Inno_VMFlags.VM_FS.getFlag()))
						| (((value1 << (value2 - 1)) & 0x80000000) != 0 ? Inno_VMFlags.VM_FC
								.getFlag()
								: 0);
				setValue(cmd.isByteMode(), mem, op1, result);
			}
			...
			case VM_SHR: {
				...
				flags = (result == 0 ? Inno_VMFlags.VM_FZ.getFlag()
						: (result & Inno_VMFlags.VM_FS.getFlag()))
						| ((value1 >>> (value2 - 1)) & Inno_VMFlags.VM_FC.getFlag());
				setValue(cmd.isByteMode(), mem, op1, result);
			}
				break;
			case VM_SAR: {
			...
			case VM_NEG: {
				int result = -getValue(cmd.isByteMode(), mem, op1);
				flags = result == 0 ? Inno_VMFlags.VM_FZ.getFlag() : Inno_VMFlags.VM_FC
						.getFlag()
						| (result & Inno_VMFlags.VM_FS.getFlag());
				setValue(cmd.isByteMode(), mem, op1, result);
			}
			...
			case VM_ADC: {
				int value1 = getValue(cmd.isByteMode(), mem, op1);
				int FC = (flags & Inno_VMFlags.VM_FC.getFlag());
				int result = (int) ((long) value1 & 0xFFffFFff
			...
			case VM_STANDARD:
				ExecuteStandardFilter(Inno_VMStandardFilters.findFilter(cmd.getOp1()
						.getData()));
				break;
			case VM_PRINT:
				break;
	...
	public void prepare(byte[] code, int codeSize, Inno_VMPreparedProgram prg) {
		InitBitInput();
		...
		prg.setCmdCount(0);
		if (xorSum == code[0]) {
			Inno_VMStandardFilters filterType = IsStandardFilter(code, codeSize);
			if (filterType != Inno_VMStandardFilters.VMSF_NONE) {

				Inno_VMPreparedCommand curCmd = new Inno_VMPreparedCommand();
				curCmd.setOpCode(Inno_VMCommands.VM_STANDARD);
				curCmd.getOp1().setData(filterType.getFilter());
				curCmd.getOp1().setType(Inno_VMOpType.VM_OPNONE);
				curCmd.getOp2().setType(Inno_VMOpType.VM_OPNONE);
				codeSize = 0;			
			...
			while (inAddr < codeSize) {
				...
				int data = fgetbits();
				if ((data & 0x8000) == 0) {
					curCmd.setOpCode(Inno_VMCommands.findVMCommand((data >> 12)));
					faddbits(4);
				} else {
					curCmd.setOpCode(Inno_VMCommands
							.findVMCommand((data >> 10) - 24));
					faddbits(6);
				}
				if ((Inno_VMCmdFlags.VM_CmdFlags[curCmd.getOpCode().getVMCommand()] & Inno_VMCmdFlags.VMCF_BYTEMODE) != 0) {
				...
				int opNum = (Inno_VMCmdFlags.VM_CmdFlags[curCmd.getOpCode()
						.getVMCommand()] & Inno_VMCmdFlags.VMCF_OPMASK);
				...
				if (opNum > 0) {
					...
					if (opNum == 2)
						...
						if (curCmd.getOp1().getType() == Inno_VMOpType.VM_OPINT
								&& (Inno_VMCmdFlags.VM_CmdFlags[curCmd.getOpCode()
										.getVMCommand()] & (Inno_VMCmdFlags.VMCF_JUMP | Inno_VMCmdFlags.VMCF_PROC)) != 0) {
		...
		Inno_VMPreparedCommand curCmd = new Inno_VMPreparedCommand();
		curCmd.setOpCode(Inno_VMCommands.VM_RET);
	...
	private void decodeArg(Inno_VMPreparedOperand op, boolean byteMode) {
		int data = fgetbits();
		if ((data & 0x8000) != 0) {
			op.setType(Inno_VMOpType.VM_OPREG);
			op.setData((data >> 12) & 7);
			op.setOffset(op.getData());
			faddbits(4);
		} else {
			if ((data & 0xc000) == 0) {
				op.setType(Inno_VMOpType.VM_OPINT);
				if (byteMode) {
				...
				op.setType(Inno_VMOpType.VM_OPREGMEM);
				if ((data & 0x2000) == 0) {
	...
	private void optimize(Inno_VMPreparedProgram prg) {
		List<Inno_VMPreparedCommand> commands = prg.getCmd();

		for (Inno_VMPreparedCommand cmd : commands) {
			switch (cmd.getOpCode()) {
			case VM_MOV:
				cmd.setOpCode(cmd.isByteMode() ? Inno_VMCommands.VM_MOVB
						: Inno_VMCommands.VM_MOVD);
				continue;
			case VM_CMP:
				cmd.setOpCode(cmd.isByteMode() ? Inno_VMCommands.VM_CMPB
						: Inno_VMCommands.VM_CMPD);
				continue;
			}
			if ((Inno_VMCmdFlags.VM_CmdFlags[cmd.getOpCode().getVMCommand()] & Inno_VMCmdFlags.VMCF_CHFLAGS) == 0) {
				continue;
			}
			boolean flagsRequired = false;

			for (int i = commands.indexOf(cmd) + 1; i < commands.size(); i++) {
				int flags = Inno_VMCmdFlags.VM_CmdFlags[commands.get(i).getOpCode()
						.getVMCommand()];
				if ((flags & (Inno_VMCmdFlags.VMCF_JUMP | Inno_VMCmdFlags.VMCF_PROC | Inno_VMCmdFlags.VMCF_USEFLAGS)) != 0) {
					flagsRequired = true;
					break;
				}
				if ((flags & Inno_VMCmdFlags.VMCF_CHFLAGS) != 0) {
					break;
				}
			}
			if (flagsRequired) {
			...
			switch (cmd.getOpCode()) {
			case VM_ADD:
				cmd.setOpCode(cmd.isByteMode() ? Inno_VMCommands.VM_ADDB
						: Inno_VMCommands.VM_ADDD);
				continue;
			case VM_SUB:
				cmd.setOpCode(cmd.isByteMode() ? Inno_VMCommands.VM_SUBB
						: Inno_VMCommands.VM_SUBD);
				continue;
			case VM_INC:
				cmd.setOpCode(cmd.isByteMode() ? Inno_VMCommands.VM_INCB
						: Inno_VMCommands.VM_INCD);
				continue;
			case VM_DEC:
				cmd.setOpCode(cmd.isByteMode() ? Inno_VMCommands.VM_DECB
						: Inno_VMCommands.VM_DECD);
				continue;
			case VM_NEG:
				cmd.setOpCode(cmd.isByteMode() ? Inno_VMCommands.VM_NEGB
						: Inno_VMCommands.VM_NEGD);
				continue;
			}
		}
	...
	public static int ReadData(Inno_BitInput rarVM) {
	...
	private Inno_VMStandardFilters IsStandardFilter(byte[] code, int codeSize) {
		Inno_VMStandardFilterSignature stdList[]={
				new Inno_VMStandardFilterSignature(53, 0xad576887, Inno_VMStandardFilters.VMSF_E8),
				new Inno_VMStandardFilterSignature(57, 0x3cd7e57e, Inno_VMStandardFilters.VMSF_E8E9),
				new Inno_VMStandardFilterSignature(120, 0x3769893f, Inno_VMStandardFilters.VMSF_ITANIUM),
				new Inno_VMStandardFilterSignature(29, 0x0e06077d, Inno_VMStandardFilters.VMSF_DELTA),
				new Inno_VMStandardFilterSignature(149, 0x1c2c5dc8, Inno_VMStandardFilters.VMSF_RGB),
				new Inno_VMStandardFilterSignature(216, 0xbc85e701, Inno_VMStandardFilters.VMSF_AUDIO),
				new Inno_VMStandardFilterSignature(40, 0x46b9c560, Inno_VMStandardFilters.VMSF_UPCASE)
		};
		int CodeCRC = Inno_RarCRC.checkCrc(0xffffffff,code,0,code.length)^0xffffffff;
		...
		return(Inno_VMStandardFilters.VMSF_NONE);
	}

	private void ExecuteStandardFilter(Inno_VMStandardFilters filterType) {
		switch(filterType)
		  {
		    case VMSF_E8:
		    case VMSF_E8E9:
		      {
		        ...
		        byte cmpByte2=(byte) ((filterType==Inno_VMStandardFilters.VMSF_E8E9) ? 0xe9:0xe8);
