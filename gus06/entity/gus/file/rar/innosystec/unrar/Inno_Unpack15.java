...
public abstract class Inno_Unpack15 extends Inno_BitInput
{
	protected int readBorder;
	protected boolean suspended;
	protected boolean unpAllBuf;
	protected Inno_ComprDataIO unpIO;
	...
	protected void unpack15(boolean solid) throws IOException, Inno_RarException
	{
		if (suspended) {
			unpPtr = wrPtr;
		} else {
	...
	protected boolean unpReadBuf() throws IOException, Inno_RarException
	{
		  int dataSize=readTop-inAddr;
		  if (dataSize<0){
		    return(false);
		  }
		  if (inAddr>Inno_BitInput.MAX_SIZE/2) {
		    if (dataSize>0){
...
//				}
		  ...
		  int readCode=unpIO.unpRead(inBuf, dataSize, (Inno_BitInput.MAX_SIZE-dataSize)&~0xf);
		  if (readCode>0){
		    readTop+=readCode;
		  }
	...
	protected void oldCopyString(int Distance, int Length)
	{
		destUnpSize -= Length;
		while ((Length--) != 0) {
			window[unpPtr] = window[(unpPtr - Distance) & Inno_Compress.MAXWINMASK];
	...
	protected void oldUnpWriteBuf() throws IOException
	{
		...
		if (unpPtr < wrPtr) {
			unpIO.unpWrite(window, wrPtr, -wrPtr & Inno_Compress.MAXWINMASK);
