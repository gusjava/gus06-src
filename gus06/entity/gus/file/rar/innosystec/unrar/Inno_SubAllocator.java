...
public class Inno_SubAllocator {
    public static final int N1 = 4, N2 = 4, N3 = 4, N4 = (128 + 3 - 1 * N1 - 2
	    * N2 - 3 * N3) / 4;

    public static final int N_INDEXES = N1 + N2 + N3 + N4;

    public static final int UNIT_SIZE = Math.max(Inno_PPMContext.size,
	    Inno_RarMemBlock.size);
    ...
    private final Inno_RarNode[] freeList = new Inno_RarNode[N_INDEXES];
    ...
    // Temp fields
    private Inno_RarNode tempRarNode = null;
    private Inno_RarMemBlock tempRarMemBlock1 = null;
    private Inno_RarMemBlock tempRarMemBlock2 = null;
    private Inno_RarMemBlock tempRarMemBlock3 = null;

    public Inno_SubAllocator() {
	clean();
    }
	...
	realAllocSize += Inno_RarMemBlock.size;
	...
	assert (realAllocSize - tempMemBlockPos == Inno_RarMemBlock.size) : realAllocSize
		+ " " + tempMemBlockPos + " " + Inno_RarMemBlock.size;

	// Init freeList
	for (int i = 0, pos = freeListPos; i < freeList.length; i++, pos += Inno_RarNode.size) {
	    freeList[i] = new Inno_RarNode(heap);
	    freeList[i].setAddress(pos);
	}

	// Init temp fields
	tempRarNode = new Inno_RarNode(heap);
	tempRarMemBlock1 = new Inno_RarMemBlock(heap);
	tempRarMemBlock2 = new Inno_RarMemBlock(heap);
	tempRarMemBlock3 = new Inno_RarMemBlock(heap);

	return true;
    }

    private void glueFreeBlocks() {
	Inno_RarMemBlock s0 = tempRarMemBlock1;
	s0.setAddress(tempMemBlockPos);
	Inno_RarMemBlock p = tempRarMemBlock2;
	Inno_RarMemBlock p1 = tempRarMemBlock3;
	int i, k, sz;
	if (loUnit != hiUnit) {
	    heap[loUnit] = 0;
	}
	...
	return freeList.length * Inno_RarNode.size;
    }
