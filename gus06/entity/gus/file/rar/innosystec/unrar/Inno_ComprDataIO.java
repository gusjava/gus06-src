...
public class Inno_ComprDataIO {

	private final Inno_Archive archive;
	private long unpPackedSize;
	private boolean testMode;
	private boolean skipUnpCRC;
	private InputStream inputStream;
	private OutputStream outputStream;
	private Inno_FileHeader subHead;
	...
	public Inno_ComprDataIO(Inno_Archive arc)
	{this.archive = arc;}
	...
	public void init(Inno_FileHeader hd) throws IOException
	{
		long startPos = hd.getPositionInFile() + hd.getHeaderSize();
		unpPackedSize = hd.getFullPackSize();
		inputStream = new Inno_ReadOnlyAccessInputStream(
				archive.getRof(),
				startPos,
	...
	public int unpRead(byte[] addr, int offset, int count) throws IOException, Inno_RarException
	{
		...
		while(count > 0)
		{
			...
			if (subHead.isSplitAfter()){
				packedCRC = Inno_RarCRC.checkCrc((int)packedCRC, addr, offset, retCode);
			}
			...
			{
				if(!Inno_Volume.mergeArchive(archive, this))
				{
		...
		if(retCode != -1) retCode = totalRead;
		return retCode;
	}
	...
	public void unpWrite(byte[] addr, int offset, int count) throws IOException
	{
		...
		if (!skipUnpCRC)
		{
			if(archive.isOldFormat())
				unpFileCRC = Inno_RarCRC.checkOldCrc((short)unpFileCRC, addr, count);
			else unpFileCRC = Inno_RarCRC.checkCrc((int)unpFileCRC, addr,offset, count);
		}
//		if (!skipArcCRC) archive.updateDataCRC(Addr, offset, ReadSize);
	}
	...
	public void setSubHeader(Inno_FileHeader hd)
	{subHead = hd;}
	...
	public long getPackedCRC() {return packedCRC;}

	public void setPackedCRC(long packedCRC) {this.packedCRC = packedCRC;}

	public long getPackFileCRC()
	{return packFileCRC;}

	public void setPackFileCRC(long packFileCRC){this.packFileCRC = packFileCRC;}

	public boolean isPackVolume(){return packVolume;}

	public void setPackVolume(boolean packVolume){this.packVolume = packVolume;}

	public long getProcessedArcSize(){return processedArcSize;}

	public void setProcessedArcSize(long processedArcSize){this.processedArcSize = processedArcSize;}

	public long getTotalArcSize(){return totalArcSize;}

	public void setTotalArcSize(long totalArcSize){this.totalArcSize = totalArcSize;}

	public long getTotalPackRead(){return totalPackRead;}

	public void setTotalPackRead(long totalPackRead){this.totalPackRead = totalPackRead;}

	public long getUnpArcSize(){return unpArcSize;}

	public void setUnpArcSize(long unpArcSize){this.unpArcSize = unpArcSize;}

	public long getUnpFileCRC(){return unpFileCRC;}

	public void setUnpFileCRC(long unpFileCRC){this.unpFileCRC = unpFileCRC;}

	public boolean isUnpVolume(){return unpVolume;}

	public void setUnpVolume(boolean unpVolume){this.unpVolume = unpVolume;}

	public Inno_FileHeader getSubHeader(){return subHead;}
