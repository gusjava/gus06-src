...
public class Inno_FileHeader extends Inno_BlockHeader {

    private final Log logger = LogFactory.getLog(Inno_FileHeader.class.getName());
    ...
    private final Inno_HostSystem hostOS;
    ...
    public Inno_FileHeader(Inno_BlockHeader bh, byte[] fileHeader) {
	super(bh);

	int position = 0;
	unpSize = Inno_Raw.readIntLittleEndian(fileHeader, position);
	position += 4;
	hostOS = Inno_HostSystem.findHostSystem(fileHeader[4]);
	position++;

	fileCRC = Inno_Raw.readIntLittleEndian(fileHeader, position);
	position += 4;

	fileTime = Inno_Raw.readIntLittleEndian(fileHeader, position);
	position += 4;
	...
	position++;
	nameSize = Inno_Raw.readShortLittleEndian(fileHeader, position);
	position += 2;

	fileAttr = Inno_Raw.readIntLittleEndian(fileHeader, position);
	position += 4;
	if (isLargeBlock()) {
	    highPackSize = Inno_Raw.readIntLittleEndian(fileHeader, position);
	    position += 4;

	    highUnpackSize = Inno_Raw.readIntLittleEndian(fileHeader, position);
	    position += 4;
	} else {
	    highPackSize = 0;
	    highUnpackSize = 0;
	...
	if (isFileHeader()) {
		...
		if (length != nameSize) {
		    length++;
		    fileNameW = Inno_FileNameDecoder.decode(fileNameBytes, length);
		}
	    } else {
	...
	if (Inno_UnrarHeadertype.NewSubHeader.equals(headerType)) {
	    ...
	    if (Inno_NewSubHeaderType.SUBHEAD_TYPE_RR.byteEquals(fileNameBytes)) {
    ...
    public Inno_HostSystem getHostOS() {
	return hostOS;
    }
	...
	return (this.flags & Inno_BlockHeader.LHD_SPLIT_AFTER) != 0;
    }
	...
	return (this.flags & Inno_BlockHeader.LHD_PASSWORD) != 0;
    }
	...
	return Inno_UnrarHeadertype.FileHeader.equals(headerType);
    }
